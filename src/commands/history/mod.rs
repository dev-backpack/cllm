...
use tabled::{settings::{peaker::PriorityMax, Width}, Table, Tabled};
...
pub async fn insert_history(input: String, output: String) -> Result<(), Box<dyn std::error::Error>> {
...
pub async fn handle_history(history: History) -> Result<(), Box<dyn std::error::Error>> {
    ...
    table.with(
        Width::wrap(width as usize).priority::<PriorityMax>()
    );
